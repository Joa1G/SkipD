...
import {
  AbstractInsightsService,
  InsightData,
} from './abstract-insights.service';
...
export class InsightsService extends AbstractInsightsService {
  ...
  public override insights = computed<InsightData>(() => {
    ...
    const minhasInstituicoes = instituicoes.filter(
      (i) => i.id_usuario === user.id
    );
    const materiasDoUsuario = materias.filter((m) =>
      minhasInstituicoes.some((i) => i.id === m.idInstituicao)
    );
    ...
    for (const materia of materiasDoUsuario) {
      const inst = minhasInstituicoes.find(
        (i) => i.id === materia.idInstituicao
      );
    ...
    const diaMaisCritico =
      Object.entries(horasPorDia).sort((a, b) => b[1] - a[1])[0]?.[0] || '';

    const diasComPoucasFaltas = Object.entries(materiasPorDia).filter(
      ([dia, materias]) =>
        materias.every((materia) => materia.status === 'Aprovado')
    );

    const horasDosDiasComPoucasFaltas = diasComPoucasFaltas
      .map(([dia]) => horasPorDia[dia])
      .filter((h) => h > 0);
    let diaMaisLeve = '';
    if (horasDosDiasComPoucasFaltas.length > 0) {
      ...
      const diasMaisLeves = diasComPoucasFaltas
        .filter(([dia]) => horasPorDia[dia] === minHoras)
        .map(([dia]) => dia);

      diaMaisLeve = diasOrdenados
        .filter((dia) => diasMaisLeves.includes(dia))
