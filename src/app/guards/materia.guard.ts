...
export const materiaGuard: CanActivateFn = (route: ActivatedRouteSnapshot) => {
  ...
  return materiaService.getMateriaById(materiaId).pipe(
    map(result => {
      ...
      const instituicoesDoUsuario = instituicaoService.instituicoes().filter(inst => inst.id_usuario === currentUser.id);

      const materiaPertenceAoUsuario = instituicoesDoUsuario.some(inst => inst.id === materia.idInstituicao);
